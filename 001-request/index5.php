...
<?php
    ...
    if(isset($_REQUEST["day"]) && isset($_REQUEST["month"]) && isset($_REQUEST["year"])){
        ...
        echo "<br /> Sinh nhật của người đó trong năm nay : " .$birthday_in_current_year;
        $dtime = DateTime::createFromFormat("d/m/Y", $birthday_in_current_year);
        $timestamp = $dtime->getTimestamp();
        echo "<br /> Sinh nhât của người đó trong năm nay theo timestamp: " . $timestamp;
        echo "<br /> Đã qua sinh nhật hay chưa:  ";

        $thoi_gian_den_sinh_nhat = 0;
        if($current_day_timestamp >  $timestamp){
            echo "<br > Ôi đã qua sinh nhật rồi";
            ...
            $next_timestamp = $next_birth->getTimestamp();

            echo "<br /> Sinh nhật của người đó trong năm sau theo timestamp: " .$next_timestamp;
            $thoi_gian_den_sinh_nhat = $next_timestamp - $current_day_timestamp;
        }
        else{
